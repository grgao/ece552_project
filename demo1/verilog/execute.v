...
module execute (opA, opB, aluop, invb, inva, immsrc, branch, aluout, setrd, brchCnd, PCaddersrc, instruction2bits, sign);
   ...
   alu ALU(.InA(opA), .InB(opB), .Cin(1'b0), .Oper(Oper), .invA(invA | inva), .invB(invB | invb), .sign(sign), .Out(aluout), .Zero(ZF), .Ofl(OF), .CF(CF));
